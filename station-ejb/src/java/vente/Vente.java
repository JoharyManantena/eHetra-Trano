...
protected EncaissementDetails [] generateDetailsEncaissements (Connection c ) throws Exception{
    ...
    protected MvtStockFille[] createMvtStockFilles(Connection c) throws Exception {
        MvtStockFille[] valiny =  new MvtStockFille[this.getVenteDetails().length];
        VenteDetails[] venteDt = this.getVenteDetails();
        for (int i = 0; i < valiny.length; i++) {
            MvtStockFille mvt = new MvtStockFille();
            mvt.setIdVenteDetail(venteDt[i].getId());
            mvt.setEntree(0);
            mvt.setSortie(venteDt[i].getQte());
            mvt.setIdProduit(venteDt[i].getIdProduit());
            valiny[i] = mvt;
        }
        return valiny;
    }
    ...
    protected MvtStock createMvtStock() throws Exception {
        ...
        md.setDesignation(this.getDesignation());
    ...
    public void createMvtStockSortie(String u, Connection c) throws Exception {
        MvtStock ms = this.createMvtStock();
        ms.setFille(this.createMvtStockFilles(c));
        ms.setLiaisonFille("idMvtStock");
        ms.createObjectMultiple(u, c);
